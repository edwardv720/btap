...
#Modified by NRC includes
# require openstudio-standards, cz_#_fdwr variables,remove_skylight variable,  set checkwall variable to default 'false', check model climate zone file before placing cz_#fdwr into variable 'wwr_after', loop to remove skylights
...
"
  ...
  def arguments(model)
    ...
    remove_skylight = OpenStudio::Ruleset::OSArgument::makeBoolArgument('remove_skylight', false)
    remove_skylight.setDisplayName('Remove skylights?')
    remove_skylight.setDefaultValue(true)
    args << remove_skylight

    cz_4_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_4_fdwr",true)
    cz_4_fdwr.setDisplayName("Climate zone 4 FDWR")
    cz_4_fdwr.setDefaultValue(0.0)
    args << cz_4_fdwr
    
    cz_5_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_5_fdwr",true)
    cz_5_fdwr.setDisplayName("Climate zone 5 FDWR")
    cz_5_fdwr.setDefaultValue(0.0)
    args << cz_5_fdwr

    cz_6_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_6_fdwr",true)
    cz_6_fdwr.setDisplayName("Climate zone 6 FDWR")
    cz_6_fdwr.setDefaultValue(0.0)
    args << cz_6_fdwr

    cz_7A_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_7A_fdwr",true)
    cz_7A_fdwr.setDisplayName("Climate zone 7A FDWR")
    cz_7A_fdwr.setDefaultValue(0.0)
    args << cz_7A_fdwr
    
    cz_7B_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_7B_fdwr",true)
    cz_7B_fdwr.setDisplayName("Climate zone 7B FDWR")
    cz_7B_fdwr.setDefaultValue(0.0)
    args << cz_7B_fdwr
    
    cz_8_fdwr = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("cz_8_fdwr",true)
    cz_8_fdwr.setDisplayName("Climate zone 8 FDWR")
    cz_8_fdwr.setDefaultValue(0.0)
    args << cz_8_fdwr    
    ...
    check_wall.setDefaultValue(false) #turned this to default to false - NRC
    args << check_wall
  ...
  def run(model, runner, user_arguments)
    ...
    if  model_hdd <3000 then
      wwr_after = runner.getDoubleArgumentValue("cz_4_fdwr",user_arguments)
    elsif (model_hdd >= 3000 && model_hdd <4000) then
      wwr_after = runner.getDoubleArgumentValue("cz_5_fdwr",user_arguments)
    elsif  (model_hdd >= 4000 && model_hdd <5000)  then
      wwr_after = runner.getDoubleArgumentValue("cz_6_fdwr",user_arguments)
    elsif  (model_hdd >= 5000 && model_hdd <6000)  then
      wwr_after = runner.getDoubleArgumentValue("cz_7A_fdwr",user_arguments)
    elsif  (model_hdd >= 6000 && model_hdd <7000) then
      wwr_after = runner.getDoubleArgumentValue("cz_7B_fdwr",user_arguments)
    elsif  (model_hdd >= 7000 ) then
      wwr_after = runner.getDoubleArgumentValue("cz_8_fdwr",user_arguments)
    else
    ...
    remove_skylight = runner.getBoolArgumentValue('remove_skylight', user_arguments)
